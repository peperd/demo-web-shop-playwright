...
test.describe("Place Order", () => {
  ...
  test("Place order with multiple products", async ({}) => {
    ...
    const priceBlueJeans = await apparelPage.getProductPrice(
      ProductNames.BlueJeans
    );
    await apparelPage.addToCart(ProductNames.BlueJeans);
    await apparelPage.navigateToTab(TopMenu.Jewelry);
    const priceBlackAndWhiteDiamondHeart = await jewelryPage.getProductPrice(
      ProductNames.BlackAndWhiteDiamondHeart
    );
    await jewelryPage.addToCart(ProductNames.BlackAndWhiteDiamondHeart);
    await jewelryPage.navigateToTab(TopMenu.Books);
    const priceBookComputingAndInternet = await booksPage.getProductPrice(
      ProductNames.BookComputingAndInternet
    );
    await booksPage.addToCart(ProductNames.BookComputingAndInternet);
    await booksPage.goToShoppingCart();
    const actualBluJeansPrice = await cartPage.getProductPrice(
      ProductNames.BlueJeans
    );
    expect.soft(actualBluJeansPrice).toBe(priceBlueJeans);
    const actualBlackAndWhiteDiamondHeart = await cartPage.getProductPrice(
      ProductNames.BlackAndWhiteDiamondHeart
    );
    expect
      .soft(actualBlackAndWhiteDiamondHeart)
      .toBe(priceBlackAndWhiteDiamondHeart);
    const actualBookComputingAndInternet = await cartPage.getProductPrice(
      ProductNames.BookComputingAndInternet
    );
    expect
      .soft(actualBookComputingAndInternet)
      .toBe(priceBookComputingAndInternet);
    const quantityBlueJeans = await cartPage.getProductQuantity(
      ProductNames.BlueJeans
    );
    expect.soft(quantityBlueJeans).toBe(1);
    const quantityBlackAndWhiteDiamondHeart = await cartPage.getProductQuantity(
      ProductNames.BlackAndWhiteDiamondHeart
    );
    expect.soft(quantityBlackAndWhiteDiamondHeart).toBe(1);
    const quantityBookComputingAndInternet = await cartPage.getProductQuantity(
      ProductNames.BookComputingAndInternet
    );
    expect.soft(quantityBookComputingAndInternet).toBe(1);
    const subTotalBlueJeans = await cartPage.getProductSubTotal(
      ProductNames.BlueJeans
    );
    expect.soft(subTotalBlueJeans).toBe(priceBlueJeans);
    const subtotalBlackAndWhiteDiamondHeart = await cartPage.getProductSubTotal(
      ProductNames.BlackAndWhiteDiamondHeart
    );
    expect
      .soft(subtotalBlackAndWhiteDiamondHeart)
      .toBe(priceBlackAndWhiteDiamondHeart);
    const subtotalBookComputingAndInternet = await cartPage.getProductSubTotal(
      ProductNames.BookComputingAndInternet
    );
    expect
      .soft(subtotalBookComputingAndInternet)
      .toBe(priceBookComputingAndInternet);
    ...
    expect
      ...
      .toBe(
        subTotalBlueJeans +
          subtotalBlackAndWhiteDiamondHeart +
          subtotalBookComputingAndInternet
      );
