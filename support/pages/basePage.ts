...
export class BasePage {
  ...
  constructor(
    page: Page,
    baseSelector: string = ".master-wrapper-page",
    baseUrl: string = "https://demowebshop.tricentis.com/"
  ) {
    this.page = page;
    ...
    await test.step("Go to shopping cart", async () => {
      ...
      await Promise.all([
        this.page.waitForURL(
          `${this.baseUrl.replace(/\/$/, "")}${TopMenuUrl.Cart}`
        ),
        this.page.waitForLoadState("domcontentloaded"),
  ...
  async openWishList(): Promise<void> {
    await test.step("Open wishlist", async () => {
      await Promise.all([
        this.page.waitForURL(
          `${this.baseUrl.replace(/\/$/, "")}${TopMenuUrl.Wishlist}`
        ),
        this.page.waitForLoadState("domcontentloaded"),
