...
export class CartItemRow {
    ...
    private removeFromCartCheckbox = (): Locator => this.page.locator('[name="removefromcart"]');
    private get unitePrice(): Locator {return this.page.locator('.product-unit-price')};
    ...
    async getProductQuantity(name: ProductNames): Promise<number> {
        await this.getProductRow(name).locator(this.quantity()).waitFor({ state: 'visible'});
